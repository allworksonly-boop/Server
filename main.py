...
@app.post("/api/superadmin/apps/{app_id}/stop")
def stop_app(
    app_id: int,
    session: Session = Depends(db),
    _: dict = Depends(require_superadmin)
...
@app.post("/api/superadmin/apps/{app_id}/start")
def start_app(
    app_id: int,
    session: Session = Depends(db),
    _: dict = Depends(require_superadmin)
...
@app.get("/api/client/app-status/{app_key}")
def client_app_status(
    app_key: str,
    request: Request,
    session: Session = Depends(db)
):
    item = (
        session.query(AppProject)
        .filter_by(app_key=app_key)
        .first()
    )
...
async function toggleApp(id,status){
  const stopping = status !== 'STOPPED';
  const action = stopping ? 'STOP' : 'START';

  if(!confirm(action + ' this app?')) return;

  const endpoint = stopping
    ? '/api/superadmin/apps/'+id+'/stop'
    : '/api/superadmin/apps/'+id+'/start';
