...
if(!token){location.href="/web/login";}

async function api(url,opt={}){
  opt.headers=Object.assign({'Authorization':'Bearer '+token,'Content-Type':'application/json'},opt.headers||{});
  let r=await fetch(url,opt);
  if(r.status===401||r.status===403){localStorage.removeItem("sa_token");location.href="/web/login";}
  return r.json();
}
